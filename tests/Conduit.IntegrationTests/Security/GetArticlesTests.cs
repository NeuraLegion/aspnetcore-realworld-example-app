...
namespace Conduit.IntegrationTests.Security
{
    public class GetArticlesTests : IAsyncLifetime
    {
        ...
        [Fact(Timeout = 40 * 60 * 1000)]
        public async Task GetArticlesSecurityTest()
        {
            await _runner
                .CreateScan(new ScanSettings
                {
                    Tests = new[] { "sqli", "excessive_data_exposure", "csrf", "business_constraint_bypass", "xss" },
